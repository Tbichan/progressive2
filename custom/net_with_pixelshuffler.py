...
def pixel_shuffler_down(x, r=2):
    N,c_in,h_in,w_in = x.shape
    c_out = int(c_in * r**2)
    h_out=int(h_in/r)
    w_out=int(w_in/r)
    x = F.reshape(x, (N, c_in, h_out, r, w_out, r))
    x = F.transpose(x, (0, 3, 5, 1, 2, 4))
    x = F.reshape(x, (N, c_out, h_out, w_out))
    return x

def pixel_shuffler_up(x, r=2):
    N,c_in,h_in,w_in = x.shape
    c_out = int(c_in / r**2)
    h_out=h_in*r
    w_out=w_in*r
    x = F.reshape(x, (N, r, r, c_out, h_in, w_in))
...
class EqualizedUpsamleConv2d(chainer.Chain):
...
class GeneratorBlock(chainer.Chain):
    def __init__(self, in_ch, out_ch, slope=0.2):
        ...
        with self.init_scope():
            ...
            self.c0 = EqualizedConv2d(out_ch, out_ch, 3, 1, 1, slope=slope)
            self.c1 = EqualizedConv2d(out_ch, out_ch, 3, 1, 1, slope=slope)
            
    def __call__(self, x):
        #h = F.unpooling_2d(x, 2, 2, 0, outsize=(x.shape[2]*2, x.shape[3]*2))
        h = pixel_shuffler_up(x)
        h = feature_vector_normalization(F.leaky_relu(self.conv1x1(h)))
...
class Generator(chainer.Chain):
    ...
    def __call__(self, z, stage):
        ...
        if int(stage)%2==0:
            ...
            #x_0 = out_prev(pixel_shuffler_up(x))
