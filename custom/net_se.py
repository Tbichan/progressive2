...
class SEBlock(chainer.Chain):
    def __init__(self, in_ch, out_ch, r=16):
        ...
        with self.init_scope():
            self.f0 = EqualizedLinear(in_ch, (in_ch+out_ch)//(2*r), initialW=irelu)
            self.f1 = EqualizedLinear((in_ch+out_ch)//(2*r), out_ch, initialW=irelu)
    ...
    def __call__(self, x):
        ...
        h = F.relu(self.f0(h))
        h = F.sigmoid(self.f1(h))
        return F.transpose(F.broadcast_to(h, (height, width, batch, self.out_ch)), (2, 3, 0, 1))
...
class GeneratorBlock(chainer.Chain):
    def __init__(self, in_ch, out_ch):
        ...
        with self.init_scope():
            ...
            self.se = SEBlock(in_ch, out_ch, r=16)
...
class Generator(chainer.Chain):
    ...
    def __call__(self, z, stage):
        ...
        if int(stage)%2==0:
            ...
            h_low, w_low = h.shape[2:]

            x_0 = out_prev(F.unpooling_2d(h, 2, 2, 0, outsize=(2*h_low, 2*w_low)))
