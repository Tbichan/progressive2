...
class GlobalConvolution2d(chainer.Chain):
    def __init__(self, in_ch, ch, ch_out, k):
        
        super(GlobalConvolution2d, self).__init__()
        with self.init_scope():
            self.conv1_k1 = EqualizedConv2d(in_ch, ch, (k, 1), (1, 1), ((k-1)//2, 0))
            self.conv2_1k = EqualizedConv2d(ch, ch_out, (1, k), (1, 1), (0, (k-1)//2))
            
            self.conv1_1k = EqualizedConv2d(in_ch, ch, (1, k), (1, 1), (0, (k-1)//2))
            self.conv2_k1 = EqualizedConv2d(ch, ch_out, (k, 1), (1, 1), ((k-1)//2, 0))
...
class EqualizedConv2d(chainer.Chain):
    def __init__(self, in_ch, out_ch, ksize, stride, pad, nobias=False, slope=0.0, gain=None):
        ...
        if isinstance(ksize, tuple):
            kernel_s = ksize[0]*ksize[1]
        else:
            kernel_s = ksize**2
            
        if gain is None:
            self.inv_c = np.sqrt(1.0/(1.0+slope**2)*2.0/(in_ch*kernel_s))
        else:
            self.inv_c = gain / np.sqrt((in_ch*kernel_s))
...
class GeneratorBlock(chainer.Chain):
    def __init__(self, in_ch, out_ch, slope=0.2):
        ...
        self.out_ch = out_ch
        with self.init_scope():
            self.c0 = GlobalConvolution2d(in_ch, out_ch//2, out_ch, k=5)
            self.c1 = GlobalConvolution2d(out_ch, out_ch//2, out_ch, k=5)
...
class DiscriminatorBlock(chainer.Chain):
    ...
    def __init__(self, in_ch, out_ch, slope=0.2):
        ...
        self.out_ch = out_ch
        with self.init_scope():
            self.c0 = GlobalConvolution2d(in_ch, in_ch//2, in_ch, k=5)
            self.c1 = GlobalConvolution2d(in_ch, out_ch//2, out_ch, k=5)
...
class Generator(chainer.Chain):
    def __init__(self, n_hidden=512, max_stage=14):
        ...
        with self.init_scope():
            ...
            self.c0 = EqualizedConv2d(n_hidden, self.R[0], 4, 1, 3, slope=slope)
    ...
    def make_hidden(self, batchsize, xp=None):
...
class Discriminator(chainer.Chain):
    def __init__(self, max_stage=14):
        ...
        with self.init_scope():
            ...
            self.out1 = EqualizedConv2d(self.R[7], self.R[7], 4, 1, 0, slope=slope)
...
def copy_param_difnet(target_link, source_link):
    """Copy parameters of a link to another link."""
    target_params = dict(target_link.namedparams())
    for param_name, param in source_link.namedparams():
        print(param_name, target_params[param_name].data.shape, param.data.shape)
        print(len(target_params[param_name].data.shape))
        if len(target_params[param_name].data.shape) != 4:
            target_params[param_name].data[:param.data.shape[0]] = param.data
        else:
            target_params[param_name].data[:param.data.shape[0],:param.data.shape[1],:,:] = param.data
